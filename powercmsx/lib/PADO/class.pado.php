    ...
    function init ( $config = [], $retry = false ) {
        ...
        try {
            ...
            if ( $this->set_names ) {
                ...
                $this->queries[] = 'SET NAMES ' . $this->dbcharset;
            }
...
*/
    ...
    function get_blob_cols ( $model, $no_prefix = false ) {
        ...
        if ( $this->stash( "blob_cols_{$model}_{$no_prefix}" ) )
            return $this->stash( "blob_cols_{$model}_{$no_prefix}" );
        $column_defs = $this->stash( "column_defs_{$model}" );
        if (! $column_defs ) {
            $column_defs = isset( $this->scheme[ $model ] )
                        && isset( $this->scheme[ $model ]['column_defs'] )
                        ? $this->scheme[ $model ]['column_defs'] : [];
            if (! isset( $this->scheme[ $model ]['column_defs'] ) ) {
                $column_defs = $this->model( $model )->_scheme['column_defs'];
            }
            $this->stash( "column_defs_{$model}", $column_defs );
        }
        $blob_cols = [];
        foreach ( $column_defs as $key => $column ) {
            if ( $column['type'] == 'blob' ) {
                if ( $no_prefix ) {
                    $blob_cols[] = $key;
                } else {
                    $blob_cols[] = "{$model}_{$key}";
                }
            }
        }
        $this->stash( "blob_cols_{$model}_{$no_prefix}", $blob_cols );
        return $blob_cols;
    }
 ...
 */
    function get_by_key ( $params, $args = [], $cols = '*' ) {
        $args = ['limit' => 1, 'get_by_key' => true ];
        $obj = $this->load( $params, $args, $cols );
 ...
 */
    function count_group_by ( $terms = [], $args = [], $extra = '' ) {
        $args['count_group_by'] = true;
        return $this->load( $terms, $args, '', $extra );
    }
