 ...
 */
    public    $modifier_funcs = [
      ...
      'strtotime'   => 'strtotime', 'title_case' => 'ucwords'];
 ...
 */
    ...
    function unregister_callback ( $name, $kind ) {
        ...
        foreach ( $callbacks as $callback ) {
            if ( $callback != $name ) {
                $unregisterd[] = $callback;
            }
        }
        $this->callbacks[ $kind ] = $unregisterd;
            ...
            if ( is_array( $var ) ) {
                foreach ( $var as $v ) {
                    if ( isset( $this->__stash['vars'][ $v ] ) ) {
                        $cached_vars['vars'][ $v ] = $this->__stash['vars'][ $v ];
                    }
                }
            } elseif ( isset( $this->__stash[ $var ] ) ) {
                $cached_vars[ $var ] = $this->__stash[ $var ];
            }
        }
 ...
 */
    function restore ( $vars = [] ) {
        ...
        foreach ( $vars as $var ) {
            if ( is_array( $var ) ) {
                foreach ( $var as $v )
                    $this->__stash['vars'][ $v ] = isset( $cache_vars['vars'][ $v ] )
                        ? $cached_vars['vars'][ $v ] : null;
            } else {
                $this->__stash[ $var ] = isset( $cached_vars[ $var ] )
                    ? $cached_vars[ $var ] : null;
            }
        }
        if ( empty( $vars ) ) {
