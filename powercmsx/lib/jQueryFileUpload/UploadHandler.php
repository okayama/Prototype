...
class UploadHandler
{
    ...
    public function generate_response($content, $print_response = true) {
        ...
        if (!isset($this->options['no_upload'])){
            ...
            $square_file = $this->square_file;
            $upload_file = $this->upload_file;
            $upload_dir = dirname( $upload_file );
            $thumbnail_dir = dirname( $thumbnail_file );
            if (is_file($thumbnail_file)) unlink($thumbnail_file);
            if (is_file($square_file)) unlink($square_file);
            $basename = preg_quote (basename($square_file));
            $parent = '..' . DS;
            $square_file = preg_replace( "/($basename$)/", "{$parent}$1", $square_file );
            if (is_file($square_file)) unlink($square_file);
            ...
            if (is_dir($thumbnail_dir)) rmdir($thumbnail_dir);
