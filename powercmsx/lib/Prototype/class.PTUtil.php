...
class PTUtil {
    ...
    function pack_revision ( $obj, $original, $changed_cols = [], $scheme = null ) {
        $app = Prototype::get_instance();
        ...
        foreach( $columns as $col => $props ) {
            ...
            if ( $prop != 'password' && $comp_old != $comp_new ) {
                $is_changed = true;
                $changed_cols[ $col ] =
        ...
        if (! empty( $changed_cols ) ) {
            ...
            $original->id( null );
            $original->save();
            if ( $orig_relations = $original->__relations ) {
            ...
            if ( $orig_metadata = $original->__meta ) {
